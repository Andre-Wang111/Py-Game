...
os.chdir(sys.path[0])
# 將目前執行檔的路徑設定為工作目錄，這樣可以確保後續載入圖片時路徑正確
# 這樣做可以避免在不同目錄下執行程式時找不到圖片檔案
# 下次忘記先問老師
...
try:
    fighter_path = os.path.join("image", "fighter_M.png")  # 組合主角圖片路徑
    if os.path.exists(fighter_path):
        fighter_img = pygame.image.load(
            fighter_path
        ).convert_alpha()  # 載入主角圖片，保留透明度
        print(f"[載入成功] {fighter_path}")
        # 取得主角圖片尺寸
        fighter_w = fighter_img.get_width()
        fighter_h = fighter_img.get_height()
        # 設定主角初始座標為視窗正中央
        fighter_x = (win_width - fighter_w) // 2
        fighter_y = (win_height - fighter_h) // 2
    else:
        fighter_img = None
        print(f"[找不到圖片] {fighter_path}")
except Exception as e:
    fighter_img = None
    print(f"[主角圖片載入失敗] {e}")
...
while True:  # 進入主遊戲迴圈，持續執行直到程式結束
    ...
    if fighter_img:  # 若主角圖片有成功載入才允許移動
        # 根據按鍵調整主角座標，並限制不超出視窗範圍
